...
def iterrator_on_blast_hsp(blast_out):
    ...
    with open(blast_out) as r_file:
        ...
        for line in r_file:
            split_line = line.rstrip().split("\t")[:12]
...
def summarize_hit_only(
    ...
    # In silix it is the maximum and not minimum
    lg_total = min(lg_aln_query, lg_aln_subject)
...
def read_blast(
    blast_out, blast_dtypes, protein_dict, option_cov="mean", option_pid="mean"
):
    """Read the blast output fmt 6 and return the value in a table.
    It allows to get all hits into one match for each subject/query pairs

    Args:
        blast_tsv (string): name of the blast_file
        blast_dtypes (list of tuples): list of tuple with (column, dtype)
        protein_dict (dict): Dict with the length of each protein
        option_cov (str, optional): Silix option for the coverage calculation. Defaults to 'mean'.
        option_pid (str, optional): Silix option for the percentage of identitities calculation. Defaults to 'mean'.

    Returns:
        pandas.dataframe: Return a dataframe with all the values needed
    """

    print("----------------")
    print("Read blast table")
    print("----------------")

    # List of rejected gene because no length
    rejected = []

    # List of final hsps
    list_dicts = []

    numline = count_hsps(blast_out=blast_out)

    # Read the blast hsp by hsp
    for sub_blast in tqdm.tqdm(
        iterrator_on_blast_hsp(blast_out=blast_out), total=numline
    ):
        ...
        # if qseqid not in protein_dict:
        #     rejected.append(qseqid)
        #     continue

        # if sseqid not in protein_dict:
        #     rejected.append(sseqid)
        #     continue

        if qseqid not in protein_dict or sseqid not in protein_dict:
            continue
        ...
        if num_HSPs == 1:
            ...
            df_hsps = prepare_df_hsps(
                list_hsps=sub_blast, blast_dtypes=blast_dtypes, HSPMIN=100
            )
        ...
        pident_blast = round(pident_blast * 100, 0)
        coverage_blast = round(coverage_blast * 100, 0)
        ...
        dict_tmp = {
            ...
            "pident": pident_blast,
            "evalue": evalue_blast,
            "coverage": coverage_blast,
            "score": score,
        }

        list_dicts.append(dict_tmp)

    # with open("info.log", "w") as log_file:
    #     for protein in set(rejected):
    #         log_file.write(f"The proteins doesn't have size {protein}\n")
